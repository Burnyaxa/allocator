...
void* get_best(size_t size) //choose the best memory area
{
	...
	{
		if ((best == nullptr || get_size(best) > get_size(pointer)) && get_size(pointer) // if best is empty or has more empty space
...
void* mem_alloc(size_t size)
{
	...
	if (get_size(pointer) > size + SIZE_H) // if allocated more than needed, create a new block
	{
...
void* mem_realloc(void* pointer, size_t size)
{
	...
	if (get_next(pointer) != nullptr && get_size(pointer) + get_size(get_next(pointer)) //if there is a next block that size_1 + size_2 >= size
		>= size)
	{
		new_header((uint8_t*)pointer + size + SIZE_H, false, size, get_size(get_next(pointer)) -
			(size - get_size(pointer))); //make a new block from the first block and a part of the second block,
		set_size(pointer, size);
		return pointer;
	}

	if (get_next(pointer) != nullptr && get_status(get_next(pointer)) == 0 &&
		get_size(pointer) + get_size(get_next(pointer)) + SIZE_H >= size) // if the second block is free, just add it to the new block
	{
		set_size(pointer, get_size(pointer) + get_size(get_next(pointer)) + SIZE_H);
		return pointer;
	}

	void* best = mem_alloc(size); //if new size is bigger than block1 + block2 + header_t
	if (best == nullptr)
	{
		return best;
	}
